...
def get_config():
    """Se obtiene la configuracion para JWT Auth.

    la cual debe de regresar un objeto de tipo AuthJWTConfig.
    """
...
def authjwt_exception_handler(request: Request, exc: AuthJWTException) -> JSONResponse:
    ...
    """
    logger.info(msg=exc.message, extra={"event.category": "auth"})  # type: ignore
    return JSONResponse(
        status_code=421,  # type: ignore
        content={"success": False, "msg": exc.message},  # type: ignore
    )
...
# Se manejan las rutas de cada modulo
