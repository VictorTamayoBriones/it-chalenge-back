...
from fastapi import APIRouter, status, Depends, Response, Path
...
from app.permissions.roles_policy import (
    roles_create,
    roles_read,
    roles_update,
    roles_delete,
...
@router.get(
    "/search/{role_name}",
    dependencies=[Depends(roles_search)],
    status_code=status.HTTP_200_OK,
    responses={200: {"model": role_schemas.ShowRoleSearch}},
    summary="Get Specific Role by Name",
)
async def search_by_name(
    role_name: str = Path(..., min_length=1, max_length=200, regex="^[A-Za-z0-9_-]+$"),
    ...
    Search a role by name and get information about it.
    ...
    - Users can access when the role is assigned
     permission **search** to the module **roles**.

    **Parameters** :
    - Access Token

    **Path Parameters**:
    - role_name: Mandatory, min characters 3, max characters 200, only letters, numbers,
     '-' and '_'.
    ...
    *Returns*
    - role: **ShowRoleSearch**  -> A ShowRoleSearch model
     with: id (UUDI Format), name, description, is_active.

    - **status code** -> 200
    """
    Authorize.jwt_required()
    return RoleActions().search_roles(db, role_name, start, limit)
...
async def delete_role(
    ...
    *Return*
    ...
    Authorize.jwt_required()
    current_user = Authorize.get_jwt_subject()
    return RoleActions().delete_one_role(db, id, current_user)
