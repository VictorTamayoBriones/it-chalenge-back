...
from app.permissions.users_policy import (
    users_create,
    users_read,
    users_update,
    users_delete,
...
@router.get(
    "/search/{email}",
    dependencies=[Depends(users_search)],
    status_code=status.HTTP_200_OK,
    responses={200: {"model": user_schemas.ShowUsers}},
    summary="Get Specific Users by email",
)
async def search_by_email(
    email: str = Path(..., min_length=1, max_length=200),
    ...
    Search a user by email and get information about it.

    - This Endpoint performs the operation of obtaining a
     list of user in the application, you can also paginate the records.
    - Users can access when the role is assigned
     permission **search** to the module **users**.

    **Parameters** :
    - Access Token

    **Path Parameters**:
    - email: string Mandatory, min length 1, max length 200.
    ...
    *Returns*
    - user: **ShowUser** -> A ShowUser model
     with: id (UUDI Format), email, status, role_id, role_name.
    - **status code** -> 200

    """
    Authorize.jwt_required()
    return UsersActions().search_roles(db, email, start, limit)
